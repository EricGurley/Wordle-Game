...
public class wordle_final {
	...
	public static void main(String[] args) {
		...
		while(playAgain) {
		    System.out.println("Select level of difficulty: easy, medium, or expert: ");
			String difficultyResponse = input.nextLine().toLowerCase();
			//sets a random secret word
			Random random = new Random();
			int index = random.nextInt(word_bank.length);    // Selects a random word from the word bank
			secretWord = (word_bank[index]); // Initializing the answer to the randomly chosen word
		    ...
		    // If the input is either yes or no, then the input is valid and the while loop ends
			else if (difficultyResponse.equals("expert")){
		        //no hint to user for expert difficulty level
		    }
		    // will restart because the while loop condition is not satisfied yet 
		    else{
...
//		    }
			...
			for (int i = 0; i < wordLength; i++) {
		        // check if we should reveal this letter
		        if (i == randomLetterIndex || i < numOfRevealedLetters) {
		            // if yes, append the letter to the result
		            revealedLetters.append(word.charAt(i));
		        } else {
		            // if no, append an underscore
